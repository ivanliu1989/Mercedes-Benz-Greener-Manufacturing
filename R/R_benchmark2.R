...
# all$Skewness = moments::skewness(all[1,])
...
# ica
# preProcValues <- preProcess(all[, all.feat], method = c("ica"), n.comp = 20)
# trainTransformed.ica <- predict(preProcValues, all[, all.feat])
...
# idx = sample(1:nrow(train.full), 3000)
...
#     xgbFit = xgb.cv(data = dtrain, nrounds = 1000, nfold = 20, param, print_every_n = 200, early_stopping_rounds = 20, verbose = 1, maximize =T)
#     
#     xgbFit <- xgb.train(param,dtrain,nrounds = xgbFit$best_iteration,print_every_n = 200, verbose = 1, maximize =T)
#     var.imp = xgb.importance(colnames(dtrain), model = xgbFit)
#     pred = predict(xgbFit, dtest, xgbFit$bestInd)
#     # regressionEvaluation(validBC$y, pred)
#     R2 <- 1 - (sum((validBC$y-pred )^2)/sum((validBC$y-mean(validBC$y))^2))
#     print(R2)
#     
# }
...
for(i in 1:5){
    ...
    trainBC = train.full
    dtrain <- xgb.DMatrix(data.matrix(trainBC[, predictors]), label = trainBC[, response])
    xgbFit = xgb.cv(data = dtrain, nrounds = 1000, nfold = 20, param, print_every_n = 100, early_stopping_rounds = 20, verbose = 1, maximize =T)
...
write.csv(fnl.submit, file = "./submissions/xgb_submit_02.csv", row.names = F)
